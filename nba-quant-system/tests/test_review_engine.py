...
# --- parse_prediction with stored picks ---

class TestParsePredictionStoredPicks:
    def test_stored_total_pick_over(self):
        """Stored total_pick '大分' in payload yields 'over'."""
        row = {
            "game_id": 10,
            "payload": {
                "total_pick": "大分",
                "spread_pick": "主队 湖人 -4.5",
                "details": {
                    "simulation": {"predicted_margin": 5.0, "predicted_total": 215.0},
                    "total_rating": {"total_confidence": 30},
                }
    ...
    def test_stored_total_pick_under(self):
        """Stored total_pick '小分' in payload yields 'under'."""
        row = {
            "game_id": 11,
            "payload": {
                "total_pick": "小分",
                "spread_pick": "客队 勇士 +4.5（受让）",
                "details": {
                    "simulation": {"predicted_margin": 5.0, "predicted_total": 215.0},
                    "total_rating": {"total_confidence": 90},
                }
            },
        }
        result = parse_prediction(row)
    ...
    def test_stored_picks_take_priority(self):
        """Stored picks override simulation-derived values."""
        row = {
            "game_id": 12,
            "payload": {
                "total_pick": "小分",
                "spread_pick": "客队 勇士 +4.5（受让）",
                "details": {
                    "simulation": {"predicted_margin": 10.0, "predicted_total": 250.0},
                    "total_rating": {"total_confidence": 99},
                }
            },
        }
        result = parse_prediction(row)
        # Stored picks override: spread should be "away" despite positive margin
        assert result["spread_pick"] == "away"
        # Total should be "under" despite high confidence and high predicted_total
        assert result["total_pick"] == "under"


# --- extract_prediction_fields with stored picks ---

class TestExtractPredictionFieldsStoredPicks:
    def test_stored_total_pick_over(self):
        """Stored total_pick '大分' in payload yields 'over'."""
        row = {
            "game_id": 10,
...
# --- run_review skips games with missing market lines ---

class TestRunReviewMissingLines:
    @patch("app.review_engine.send_message")
    @patch("app.review_engine.fetch_game_result")
    @patch("app.review_engine.load_latest_predictions")
    def test_skips_game_without_market_lines(
        self, mock_load, mock_fetch, mock_send
    ):
        """run_review skips games when market lines are missing from payload."""
        mock_load.return_value = [
            {
                "game_id": 99,
                "payload": {
        ...
        with patch("app.supabase_client.save_review_result") as mock_save, \
             patch("app.supabase_client.fetch_recent_review_results", return_value=[]):
            run_review()

        # Game should be skipped — no review result saved
